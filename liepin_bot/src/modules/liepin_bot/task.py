...
logger = LoggerFactory.get_logger("liepin_bot")

# 账号配置
USERNAME = Config.get("LIEPIN_USERNAME")
PASSWORD = Config.get("LIEPIN_PASSWORD")
HUMAN_INTERVENTION = Config.get("HUMAN_INTERVENTION", 'true').lower() == 'true'

# 路径配置
INPUT_EXCEL = (DATA_DIR / "Input" / "Liepin" / "test_search.xlsx").resolve().as_posix()
OUTPUT_EXCEL = (DATA_DIR / "Output" / "Get_result_leipin.xlsx").resolve().as_posix()
AUTH_FILE = (CONFIG_DIR / "auth.json").resolve().as_posix()
...
class LiepinBot:
    ...
    async def init_browser(self, use_cache=False):
        ...
        # 统一注入隐匿配置，调整为相对当前模块位置寻找 stealth.min.js
...
def run(params: dict = None) -> dict:
    ...
    bot = LiepinBot()
    asyncio.run(bot.run())
    return {"status": "success", "message": "Crawler execution finished."}
