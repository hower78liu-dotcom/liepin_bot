...
class LiepinResumeExtractor:
    ...
    async def fill_search_condition(self, condition):
        ...
        try:
            ...
            dump_filename = f"waf_crash_dump_{int(time.time())}.html"
            with open(dump_filename, "w", encoding="utf-8") as f:
                f.write(page_content)
            await self.page.screenshot(path="waf_crash_screenshot.png", full_page=True)
        ...
        if target_text:
             ...
             # 截图定位前状态用于 debug
             temp_snap = f"debug_before_search_input.png"
             await self.page.screenshot(path=temp_snap)
             ...
             if await main_input.count() > 0:
                 ...
                 page_content = await self.page.content()
                 with open("debug_input_error.html", "w", encoding="utf-8") as f:
                     f.write(page_content)
        ...
        if pd.notna(condition.get("工作城市")):
            ...
            if await city_label.is_visible():
                ...
                if 'active' in classes or 'selected' in classes or 'checked' in classes:
                    logger.info(f"✅ 成功勾选城市: {city_name} (真激活)")
                else:
                    logger.warning(f"⚠️ 城市 {city_name} 被点击，但未检测到激活样式！")
            else:
                other_btn = city_block.locator('.btn-choose:has-text("其他")').first
                if await other_btn.is_visible():
                    await self._safe_click(other_btn)
                    await asyncio.sleep(1)
                    target_city = self.page.locator(f'.antd-lp-city-block label:has-text("{city_name}")').first
                    if await target_city.is_visible():
                        await self._safe_click(target_city)
                        await asyncio.sleep(0.5)
                        confirm_btn = self.page.locator('.antd-fd-city-modal button:has-text("确定"), .ant-modal-content button:has-text("确定")').first
                        if await confirm_btn.is_visible():
                            await confirm_btn.click(force=True)
                        logger.info(f"✅ 成功通过辅助面板勾选城市: {city_name}")
        ...
        if pd.notna(condition.get("活跃度")):
            ...
            act_title = self.page.locator('span:has-text("活跃度：")').first
            if await act_title.is_visible():
                ...
                if await selector_box.is_visible():
                    try:
                        ...
                        await selector_box.click(force=True)
                        # 等待 Append 到 Body 远端的 AntD 浮层面板动画生成
                        await self.page.wait_for_selector('.ant-select-dropdown:visible', timeout=3000)
                        
                        act_option = self.page.locator('.ant-select-dropdown:visible').locator(f'.ant-select-item-option-content:has-text("{act_str}")').first
                        ...
                        logger.error(f"❌ [ERROR] 点击“活跃度”下拉框交互失败或超时: {e}")
        ...
        try:
            ...
            if await tags_box.is_visible():
                ...
                if not combined_tag_str.strip():
                    ...
                    dump_path = f"failed_tags_{int(time.time())}.png"
                    await self.page.screenshot(path=dump_path, full_page=True)
    ...
    async def process_list_and_save(self, condition):
        ...
        for i in range(fetch_len):
            ...
            try:
                ...
                drawer_locator = self.page.locator('.ant-drawer, .resume-detail, [class*="drawer"], [class*="detail-content"]').first
                try:
                    await drawer_locator.wait_for(state='visible', timeout=8000)
                    detail_success = True
                except Exception:
